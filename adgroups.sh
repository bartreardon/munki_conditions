...
AD_DOMAIN=$(dsconfigad -show | grep "Directory Domain" | awk -F"= " '{print $2}' | awk -F"." '{print $1}' | tr [a-z] [A-Z])

CONSOLE_USER=$(who | grep console | cut -d ' ' -f1)
USER_AD_GROUPS=$(dscl /Active\ Directory/$AD_DOMAIN/All\ Domains -read /Users/$CONSOLE_USER dsAttrTypeNative:memberOf | sed -e 's/.*CN=//g;s/,OU=.*//g')
IFS=$'\n'
